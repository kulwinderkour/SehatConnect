...
export default function AppointmentManagementScreen() {
  ...
  const filteredAppointments = useMemo(() => {
    let filtered = appointmentState.appointments.map((a) => ({
      ...a,
      // Map status strings to match the mock shape used in UI. Keep as-is if already matching.
      status: (a.status || '').toString()
    }));

    // Status filter
    if (selectedStatus !== 'All') {
      filtered = filtered.filter(apt => apt.status === selectedStatus || apt.status?.toLowerCase() === selectedStatus.toLowerCase());
    }

    return filtered;
  }, [selectedStatus, appointmentState.appointments]);
