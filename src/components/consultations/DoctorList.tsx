...
const DoctorList: React.FC<DoctorListProps> = memo(({
  ...
  const filteredDoctors = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter(doctor => {
        // Normalize doctor name by removing periods and extra spaces
        const normalizedName = doctor.name.toLowerCase().replace(/\./g, '').replace(/\s+/g, ' ');
        const normalizedQuery = query.replace(/\./g, '').replace(/\s+/g, ' ');
        
        // Check if name contains the search query (normalized)
        const nameMatch = normalizedName.includes(normalizedQuery);
        
        // Check if individual words in the query match words in the name
        ...
        // Check specialty and hospital
        const specialtyMatch = doctor.specialty.toLowerCase().includes(query);
        const hospitalMatch = doctor.hospital && doctor.hospital.toLowerCase().includes(query);
        
        return nameMatch || wordMatch || specialtyMatch || hospitalMatch;
      });
    }
  ...
  const renderDoctor = useCallback(({ item }: { item: Doctor }) => (
    ...
    />
  ), [onDoctorPress, onConsultPress, variant, actionLabel]);
  ...
  const renderHorizontalDoctor = useCallback(({ item }: { item: Doctor }) => (
    <DoctorCard
      ...
      actionLabel={actionLabel}
    />
  ), [onDoctorPress, onConsultPress, actionLabel]);
    ...
    <View style={styles.container}>
      ...
      {filteredDoctors.length > 0 ? (
        <View style={styles.verticalList}>
          ...
          {showInternalMoreButton && filteredDoctors.length > 2 && (
            <TouchableOpacity 
            ...
            >
              <View style={styles.showMoreButtonContent}>
                ...
                {showAllDoctors ? (
                  <ChevronUp size={18} color="#5a9e31" strokeWidth={2.5} />
                ) : (
                  <ChevronDown size={18} color="#5a9e31" strokeWidth={2.5} />
                )}
              </View>
            </TouchableOpacity>
          )}
